...
char *argv[];
{
  ...
  if(PEXOR_RX(&sPEXOR_1, l_sfp, &a , &b, &c)==-1) {
    ...
    //    printf ("Reply: 0x%x 0x%x 0x%x %d \n", a,b,c,c);
    if((a&0xfff)!=PEXOR_PT_AD_R_REQ && 
       (a&0xfff)!=PEXOR_PT_AD_W_REQ && 
       (a&0xfff)!=PEXOR_INI_REQ && 
       (a&0xfff)!=PEXOR_PT_AD_R_REP && 
       (a&0xfff)!=PEXOR_PT_AD_W_REP ) {
	      printf ("ERROR: Packet Structure : Command Reply:0x%x  Address:0x%x Data:0x%x \n", a,b,c);
    }
  }

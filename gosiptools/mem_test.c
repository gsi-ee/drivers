...
char *argv[];
{
  ...
  l_comm = 0x44| (0x1<<16+l_sfp);
  addr_i = l_addr;
  data_i = l_data;
  //  printf ("command address data : 0x%x 0x%x 0x%x  \n", l_comm, l_addr, l_data);

  for(i=0;i<loop;i++){
    PEXOR_TK_Mem_Write ( &sPEXOR_1, (long*) &l_addr,&l_data);
    PEXOR_TK_Mem_Read ( &sPEXOR_1, (long) &l_addr,(long **) &l_rdata);
    printf ("address w_data r_data : 0x%x 0x%x 0x%x  \n", l_addr, l_data, l_rdata);
    l_addr=l_addr+0x04;
    //    l_data=l_data+0x1;
  }

...
char *argv[];
{
  ...
  long l_rw;
  long l_rd_dat;
  long l_sfp, l_slave, l_address,l_comm,l_loop;
  ...
  if (argc!=5){
    printf ("\nusage: read_slave ");
    printf ("sfp[0-3] slave[0-3] address[0-FFFFFC] loop[1-0x1000]\n");
    exit (0);
  }  else  {
    sscanf (argv[1], "%x", &l_sfp);
    sscanf (argv[2], "%x", &l_slave);
    sscanf (argv[3], "%x", &l_address);
    sscanf (argv[4], "%x", &l_loop);
  }
  ...
  for(i=0;i<l_loop ;i++){
...
#endif
    PEXOR_TX(&sPEXOR_1,  l_comm, l_address, 0x0) ;
    //    sleep(1);
	...
	printf ("Module: 0x%x Address: 0x%x  Data: 0x%x \n", ( b&0xff000000) >> 24 , b&0xffffff ,c);
	if(a&0x4000!=0){
	  printf ("ERROR: Packet Structure : Command Reply 0x%x \n", a);
	}
      } else {
	printf ("ERROR : Access to empty slave or address: Module  0x%x Address 0x%x  Command Reply  0x%x \n", ( l_address&0xff000000) >> 24 , l_address&0xffffff, a );
      }
    }

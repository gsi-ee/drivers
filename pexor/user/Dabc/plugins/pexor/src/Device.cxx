...
pexorplugin::Device::Device(Basic* parent, const char* name, dabc::Command* cmd) :
dabc::Device(parent, name),fBoard(0),fMbsFormat(true), fTestData(true),
...
{
	fDeviceNum = GetCfgInt(pexorplugin::xmlPexorID, 0, cmd);
	 ...
	 for (int sfp=0; sfp<PEXORPLUGIN_NUMSFP; sfp++)
		 {
			 ...
			 fNumSlavesSFP[sfp]=GetCfgInt(FORMAT(("%s%d",xmlPexorSFPSlaves, sfp)), false,cmd);
   ...
   fSynchronousRead=GetCfgBool(pexorplugin::xmlSyncRead,true, cmd);

   fParallelRead=GetCfgBool(pexorplugin::xmlParallelRead,false, cmd);

   fTriggeredRead=GetCfgBool(pexorplugin::xmlTriggeredRead,false, cmd);

   fTrixConvTime=GetCfgInt(pexorplugin::xmlTrixorConvTime,0x200, cmd);
   fTrixFClearTime=GetCfgInt(pexorplugin::xmlTrixorFastClearTime,0x100, cmd);
...
bool pexorplugin::Device::DoDeviceCleanup(bool full)
{
   DOUT1(("_______pexorplugin::Device::DoDeviceCleanup...%d ",full));
...
int pexorplugin::Device::CreateTransport(dabc::Command* cmd, dabc::Port* port)
{
   ...
   DOUT3(("pexorplugin::Device::CreateTransport creates new transport instance %p", transport));
   DOUT3(("Device thread %p\n", ProcessorThread()));

   transport->AssignProcessorToThread(ProcessorThread());
   transport->AttachPort(port,true);
      ...
      if ((fMemoryTest && !fSkipRequest) || !fMemoryTest)
        {
          ...
          DOUT5(("pexorplugin::Device::ReceiveAllTokenBuffer got token buffer of len %d\n", oldbuflen));
        }
      else
        {
          ...
          DOUT5(("pexorplugin::Device::ReceiveAllTokenBuffer set dummy buffer len to %d\n", oldbuflen));
        }
    }
  if(!fSkipRequest)

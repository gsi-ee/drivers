...
void usage ()
{
  printf ("\n**** polandtest v 0.3 30-Jun-2030 by JAM (j.adamczewski@gsi.de)\n");
  printf ("* read out data via token DMA from preconfigured poland frontends and unpack/display\n");
  printf ("Usage:\n");
  printf ("\t polandtest [-i] [-s sfp] [-p slaves ] [-b bufsize] [-n numbufs] [-t numtrigs] [-e numevts] [-d level]\n");
  printf ("Options:\n");
  ...
  printf ("\t\t -e numevts  : set number of events to read (default 100, overruled by -t option) \n");
...
int main (int argc, char **argv)
{
  ...
  /* get arguments*/
  int opt, optind = 1;
  while ((opt = getopt (argc, argv, "hs:p:b:n:t:e:xd:i")) != -1)
  {
    switch (opt)
    {
      case '?':
...
//    printf(" - set data value:0x%x\n",Data);
  ...
  Trignum = (Trignum / 2) * 2; /** Be sure to always read even number of events, otherwise we get trouble with double buffer flip*/

  /** Main READOUT LOOP HERE:*/
  loopbench.ClockStart ();
  loopbench.TimerStart ();
  int loopbytes=0;
  printf("\n BEGIN of readout loop for %d events... \n",Trignum);
...
//				bench.ShowRate("Cycles: token read:", tokensize , cycledelta);
  ...
  double lcycledelta = loopbench.TimerDelta ();
  double lclockdelta = loopbench.ClockDelta ();
  printf(" END of readout loop after %d events: read %d bytes in %f s  \n",Trignum, loopbytes, lclockdelta);
  loopbench.ShowRate ("Clock:  Read all events:", loopbytes, lclockdelta);    // bytes
  loopbench.ShowRate ("Cycles: Read all events:", loopbytes , lcycledelta);
  printf("\n Shutting down now... \n\n");

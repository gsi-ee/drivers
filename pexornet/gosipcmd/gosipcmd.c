...
{
  com->sd_pex = pexornet_open (com->devnum);
  if (com->sd_pex == 0)
  {
    printm ("ERROR>> open pexornet interface %d \n", com->devnum);
    exit (1);
  }
  return 0;
}
...
int goscmd_write (struct gosip_cmd* com)
{
  ...
  if (com->verboselevel)
    goscmd_dump_command (com);
  rev = pexornet_slave_wr (com->sd_pex, com->sfp, com->slave, com->address, com->value);
  if (rev != 0)
  {
...
int goscmd_read (struct gosip_cmd* com)
{
  ...
  if (com->verboselevel > 1)
    goscmd_dump_command (com);
  rev = pexornet_slave_rd (com->sd_pex, com->sfp, com->slave, com->address, &(com->value));
  if (rev == 0)
  {
...
int goscmd_init (struct gosip_cmd* com)
{
  ...
  if (com->verboselevel)
    goscmd_dump_command (com);
  return (pexornet_slave_init (com->sd_pex, com->sfp, com->slave));
}
...
#ifdef GOSIPCMD_BLOCKCONFIG
  rev = pexornet_slave_config (com->sd_pex, &theConfig);
#endif

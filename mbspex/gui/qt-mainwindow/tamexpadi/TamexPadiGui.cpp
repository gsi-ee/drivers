...
// enable this define to show original tabbed tamex gui as single mdi subwindow
//#define TAMEX_USE_SINGLE_SUBWINDOW 1
    ...
    fShowAmplifiedVoltages (true), fTamexDumpMode (false), GosipGui (parent)
{
  ...
  fVersionString = "Welcome to TAMEX-PADI GUI!\n\t v0.94 of 30-July-2019 by JAM (j.adamczewski@gsi.de)";

  fSettings=new QSettings("GSI", fImplementationName);



#ifdef TAMEX_USE_SINGLE_SUBWINDOW
  fTamexPadiWidget=new TamexPadiWidget(this);
mdiArea->addSubWindow(fTamexPadiWidget); // complete tamex widget in one window

#else

fTamexPadiWidget=new TamexPadiWidget(0);

  QWidget* tdctab=fTamexPadiWidget->TamexPadiTabs->widget(0);
  QWidget* channeltab=fTamexPadiWidget->TamexPadiTabs->widget(1);
  QWidget* threstab=fTamexPadiWidget->TamexPadiTabs->widget(2);
  Qt::WindowFlags wflags= Qt::CustomizeWindowHint | Qt::WindowMinMaxButtonsHint | Qt::WindowTitleHint;
  if(tdctab)
    {
      fTamexPadiWidget->TamexPadiTabs->removeTab(0);
      tdctab->setWindowTitle("TDC");
      tdctab->show();
      QMdiSubWindow* subtrace=mdiArea->addSubWindow(tdctab,wflags);
      subtrace->setAttribute(Qt::WA_DeleteOnClose, false);
  ...
  if(channeltab)
    {
    fTamexPadiWidget->TamexPadiTabs->removeTab(0);
    channeltab->setWindowTitle("Channels");
    channeltab->show();
    QMdiSubWindow* subchannel=mdiArea->addSubWindow(channeltab, wflags);
  ...
  {
    fTamexPadiWidget->TamexPadiTabs->removeTab(0);
    threstab->setWindowTitle("Thresholds");
    threstab->show();
    QMdiSubWindow* subthres=mdiArea->addSubWindow(threstab, wflags);
...
void TamexPadiGui::RefreshModuleCaps (Tamex_Module_type mod)
{
  ...
  // TODO JAM2019 - handle this for subwindow mdi mode?

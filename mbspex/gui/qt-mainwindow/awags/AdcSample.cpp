...
void AdcSample::Reset ()
{
  ...
  fMaxValue = 0;
  fSample.clear();
  //fSample.resize(AWAGS_ADC_SAMPLEVALUES);

  fBaselineStart=0;
  fBaselineStop=AWAGS_ADC_SAMPLEVALUES;
  fMean=AWAGS_NOVALUE;
  fSigma=AWAGS_NOVALUE;
  fPeaks.clear();
  fPeakDelta=0;
  fHeightDelta=0;
  fNegative=false;
...
int  AdcSample::GetPeakHeight(int num)
 {
  if(!IsValid() || num >= GetNumPeaks()) return AWAGS_NOVALUE;
...
void AdcSample::FindPeaks(double deltaratio, double falldistance, bool negative)
 {
    int peak[AWAGS_ADC_NUMMAXIMA];
    int pos[AWAGS_ADC_NUMMAXIMA];
  ...
  for(int p=0; p<AWAGS_ADC_NUMMAXIMA;++p)
  {
    pos[p]=0;
    peak[p]=0;
    if(negative) peak[p]=-AWAGS_ADC_MAXVALUE;

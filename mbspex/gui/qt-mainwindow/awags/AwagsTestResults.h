#ifndef AWAGSTESTRESULTS_H
#define AWAGSTESTRESULTS_H
...
#define ASSERT_TEST_VALID   if(checkvalid && !fValid) return AWAGS_NOVALUE;

//#define ASSERT_TEST_VALID ;
/** keeps the measured resulsts of a benchmark scan */
class AwagsTestResults
{
private:
  ...
  /** the address id of this awags chip on the board*/
   ...
   /** the absolute values of the AWAGS dacs after autocalibration*/
   uint16_t fDACValueCalibrate[AWAGS_NUMDACS];


   /** keep results of DAC-ADC calibration mapped to awags channels*/
   GainSetup fDAC_ADC_Gain[AWAGS_NUMDACS];

   /** keep measured work curve*/
   DacWorkCurve fDAC_Curve[AWAGS_NUMDACS];

   /** mean value of measured baseline after autocalibration*/
   double fMean[AWAGS_NUMDACS];

   /** sigma value of measured baseline after autocalibration*/
   double fSigma[AWAGS_NUMDACS];

   /** remember lower limit of baseline region*/
   int fBaselineLowerBound[AWAGS_NUMDACS];

   /** remember Upper limit of baseline region*/
   int fBaselineUpperBound[AWAGS_NUMDACS];


   /** keep minimum value of baseline sample set*/
    uint16_t fMinValue[AWAGS_NUMDACS];;
...
public:
  ...
  /** set the DAC value val after AWAGS chip autocalibration for dac unit 0...3*/
  int SetDacValueCalibrate(int dac, uint16_t val);

  /** get the DAC value after AWAGS chip autocalibration for dac 0...3.
   * checkvalid flag checks if test was completely finished**/
  int GetDacValueCalibrate(int dac, bool checkvalid=false);

  /** set the sampled mean ADC value after AWAGS chip autocalibration for dac 0...3*/
  int SetAdcSampleMean(int dac, double val);

  /** get the sampled mean Adc value after AWAGS chip autocalibration for dac 0...3*/
  double GetAdcSampleMean(int dac, bool checkvalid=false);

  /** set the sampled ADC sigma value after AWAGS chip autocalibration for dac 0...3*/
  int SetAdcSampleSigma(int dac, double val);

  /** get the sampled ADC sigma value after AWAGS chip autocalibration for dac 0...3*/

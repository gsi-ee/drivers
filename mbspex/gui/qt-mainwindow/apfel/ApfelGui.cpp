...
//#define APFEL_USE_SINGLE_SUBWINDOW 1
        ...
        fPlotMaxDac (APFEL_DAC_MAXVALUE), fPlotMinAdc (0), fPlotMaxAdc (APFEL_ADC_MAXVALUE), fUseSimpleSwitchAddressing(false)
{
  ...
  fVersionString="Welcome to APFEL GUI!\n\t v0.99972 of 01-July-2020 by JAM (j.adamczewski@gsi.de)\n";

  fSettings=new QSettings("GSI", fImplementationName);



#ifdef APFEL_USE_SINGLE_SUBWINDOW
fApfelWidget=new ApfelWidget(this);
mdiArea->addSubWindow(fApfelWidget); // complete febex widget in one window

#else

  fApfelWidget=new ApfelWidget(0);

  QWidget* iotab=fApfelWidget->ApfelTabWidget->widget(0);
  QWidget* gaintab=fApfelWidget->ApfelTabWidget->widget(1);
  QWidget* dactab=fApfelWidget->ApfelTabWidget->widget(2);
  QWidget* pulsertab=fApfelWidget->ApfelTabWidget->widget(3);
  QWidget* basetab=fApfelWidget->ApfelTabWidget->widget(4);
  QWidget* sampletab=fApfelWidget->ApfelTabWidget->widget(5);
  QWidget* plottab=fApfelWidget->ApfelTabWidget->widget(6);
  QWidget* addresstab=fApfelWidget->ApfelTabWidget->widget(7);
  QWidget* currenttab=fApfelWidget->ApfelTabWidget->widget(8);
  QWidget* charactertab=fApfelWidget->ApfelTabWidget->widget(9);


  Qt::WindowFlags wflags= Qt::CustomizeWindowHint | Qt::WindowMinMaxButtonsHint | Qt::WindowTitleHint;
  if(iotab)
    {
      fApfelWidget->ApfelTabWidget->removeTab(0);

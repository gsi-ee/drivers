...
//#define FEBEX_USE_SINGLE_SUBWINDOW 1
...
mdiArea->addSubWindow(fFebexWidget); // complete febex widget in one window

#else

  fFebexWidget=new FebexWidget(0);

  QWidget* tracetab=fFebexWidget->FebexTabs->widget(0);
  QWidget* channeltab=fFebexWidget->FebexTabs->widget(1);
  QWidget* threstab=fFebexWidget->FebexTabs->widget(2);
  QWidget* baselinetab=fFebexWidget->FebexTabs->widget(3);
  Qt::WindowFlags wflags= Qt::CustomizeWindowHint | Qt::WindowMinMaxButtonsHint | Qt::WindowTitleHint;
  if(tracetab)
    {
      fFebexWidget->FebexTabs->removeTab(0);
      tracetab->setWindowTitle("Traces");
      tracetab->show();
      QMdiSubWindow* subtrace=mdiArea->addSubWindow(tracetab,wflags);
  ...
  {
    fFebexWidget->FebexTabs->removeTab(0);
    baselinetab->setWindowTitle("Baslines TUM");
    baselinetab->show();
    QMdiSubWindow* subbase= mdiArea->addSubWindow(baselinetab, wflags);

...
// enable this define to show original tabbed nyxor gui as single mdi subwindow
//#define NYXOR_USE_SINGLE_SUBWINDOW 1
...
#ifdef NYXOR_USE_SINGLE_SUBWINDOW




  fNyxorWidget=new NyxorWidget(this);
  mdiArea->addSubWindow(fNyxorWidget); // complete tamex widget in one window
...
// here components for (nxyter and other blocks) tabs:
  ...
  fGeneralTab = new GeneralNyxorWidget (this, this);
  fADCTab =  new NyxorADCWidget (this, this);
  fDACTab =  new NyxorDACWidget (this, this);

  Qt::WindowFlags wflags= Qt::CustomizeWindowHint | Qt::WindowMinMaxButtonsHint | Qt::WindowTitleHint;

  fGeneralTab->setWindowTitle("Receiver");
  fGeneralTab->show();
  QMdiSubWindow* subgen=mdiArea->addSubWindow(fGeneralTab,wflags);
  subgen->setAttribute(Qt::WA_DeleteOnClose, false);


  fADCTab->setWindowTitle("ADC");
  fADCTab->show();
  QMdiSubWindow* subadc=mdiArea->addSubWindow(fADCTab, wflags);
  subadc->setAttribute(Qt::WA_DeleteOnClose, false);

  fDACTab->setWindowTitle("DACs");
  fDACTab->show();
  QMdiSubWindow* subdac=mdiArea->addSubWindow(fDACTab, wflags);
  subdac->setAttribute(Qt::WA_DeleteOnClose, false);


  for (int nx = 0; nx < NYXOR_NUMNX; nx++)
    {
      NxyterWidget* nxw = new NxyterWidget (this, this, nx);
      nxw->show();
      nxw->setWindowTitle(QString ("NX%1").arg (nx));
      QMdiSubWindow* subnx=mdiArea->addSubWindow(nxw,wflags);
      subnx->setAttribute(Qt::WA_DeleteOnClose, false);
      fNxTab[nx] = nxw;
    }

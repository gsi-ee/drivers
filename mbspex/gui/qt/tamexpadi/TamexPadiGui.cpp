...
TamexPadiGui::TamexPadiGui (QWidget* parent) : fShowAmplifiedVoltages(true), GosipGui (parent)
{
 ...
 fVersionString="Welcome to TAMEX-PADI GUI!\n\t v0.71 of 08-Feb-2018 by JAM (j.adamczewski@gsi.de)";
  ...
  QObject::connect (fTamexPadiWidget->Threshold_Slider_00, SIGNAL(valueChanged(int)), this, SLOT(Threshold_Slider_00_changed(int)));
    ...
    QObject::connect (fTamexPadiWidget->Threshold_Value_all, SIGNAL(editingFinished()), this, SLOT(Threshold_Text_all_changed()));



    QObject::connect (fTamexPadiWidget->Threshold_Value_00, SIGNAL(editingFinished()), this, SLOT(Threshold_Text_00_changed()));
    QObject::connect (fTamexPadiWidget->Threshold_Value_01, SIGNAL(editingFinished()), this, SLOT(Threshold_Text_01_changed()));
    QObject::connect (fTamexPadiWidget->Threshold_Value_02, SIGNAL(editingFinished()), this, SLOT(Threshold_Text_02_changed()));
    QObject::connect (fTamexPadiWidget->Threshold_Value_03, SIGNAL(editingFinished()), this, SLOT(Threshold_Text_03_changed()));
    QObject::connect (fTamexPadiWidget->Threshold_Value_04, SIGNAL(editingFinished()), this, SLOT(Threshold_Text_04_changed()));
    QObject::connect (fTamexPadiWidget->Threshold_Value_05, SIGNAL(editingFinished()), this, SLOT(Threshold_Text_05_changed()));
    QObject::connect (fTamexPadiWidget->Threshold_Value_06, SIGNAL(editingFinished()), this, SLOT(Threshold_Text_06_changed()));
    QObject::connect (fTamexPadiWidget->Threshold_Value_07, SIGNAL(editingFinished()), this, SLOT(Threshold_Text_07_changed()));
    QObject::connect (fTamexPadiWidget->Threshold_Value_08, SIGNAL(editingFinished()), this, SLOT(Threshold_Text_08_changed()));
    QObject::connect (fTamexPadiWidget->Threshold_Value_09, SIGNAL(editingFinished()), this, SLOT(Threshold_Text_09_changed()));
    QObject::connect (fTamexPadiWidget->Threshold_Value_10, SIGNAL(editingFinished()), this, SLOT(Threshold_Text_10_changed()));
    QObject::connect (fTamexPadiWidget->Threshold_Value_11, SIGNAL(editingFinished()), this, SLOT(Threshold_Text_11_changed()));
    QObject::connect (fTamexPadiWidget->Threshold_Value_12, SIGNAL(editingFinished()), this, SLOT(Threshold_Text_12_changed()));
    QObject::connect (fTamexPadiWidget->Threshold_Value_13, SIGNAL(editingFinished()), this, SLOT(Threshold_Text_13_changed()));
    QObject::connect (fTamexPadiWidget->Threshold_Value_14, SIGNAL(editingFinished()), this, SLOT(Threshold_Text_14_changed()));
    QObject::connect (fTamexPadiWidget->Threshold_Value_15, SIGNAL(editingFinished()), this, SLOT(Threshold_Text_15_changed()));
...
void TamexPadiGui::VoltageModeCheckBoxChanged(int on)
{
 ...
 double allvolts=GetVoltageFromRegister(allval);
...
void TamexPadiGui::ChannelEnabled_toggled (int channel, bool on)
{
  ...
  {
    EvaluateSlave ();
    bool leading=fChannelLeadingRadio[channel]->isChecked();
    bool trailing=fChannelTrailingRadio[channel]->isChecked();
    if(!on)
    {
      leading = trailing = false;
    }
    GOSIP_BROADCAST_ACTION(ApplyChannelEnabled(channel, (leading ? 1:0), (trailing ? 1:0 )) );
  }
...
void TamexPadiGui::ChannelEnabled_toggled_00 (bool on)
{
 ...
 void TamexPadiGui::ChannelEnabled_toggled_all(bool on)
 {
   ...
   if (IsAutoApply () && !fBroadcasting)
      {
         EvaluateSlave ();
         GOSIP_BROADCAST_ACTION(ApplyChannelEnabledAll(on));
      }
...
void TamexPadiGui::LeadingEnabled_toggled (int channel, bool on)
{
  ...
  {
    EvaluateSlave ();
    GOSIP_BROADCAST_ACTION(ApplyChannelEnabled(channel, on, -1));
  }
...
void TamexPadiGui::ChannelLeading_toggled_all(bool on)
{
  ...
  {
    EvaluateSlave ();
    GOSIP_BROADCAST_ACTION(ApplyLeadingEnabledAll(on));
  }
...
void TamexPadiGui::TrailingEnabled_toggled (int channel, bool on)
{
  ...
  {
    EvaluateSlave ();
    GOSIP_BROADCAST_ACTION(ApplyChannelEnabled(channel, -1, on ));
  }
...
void TamexPadiGui::ChannelTrailing_toggled_all(bool on)
{
  ...
  // since we lock the slots of the radiobuttons, we handle the autoapply here separately (and better):
   ...
   {
     EvaluateSlave ();
         GOSIP_BROADCAST_ACTION(ApplyTrailingEnabledAll(on));
   }
...
// display setup structure to gui:
  ...
  int ix=0;
  switch (theSetup->GetClockSource ())
  {
    case 0x20:
      ix = 0;
      break;
...
////////////////////////////////////7
  ...
  else
  {
    ...
    // TODO: we should also disable the clock feeding to trbus here when we switch back?
  }

  if ((clksrc == 0x22) && (fSlave == 0))    // If clock from TRBus used
  {
    WriteGosip (fSFP, fSlave, REG_TAM_BUS_EN, 0x80);    // Enable feeding clock to TRBus on slave 0
  }
  else
  {
    // TODO: we should disable the clock feeding to trbus here when we switch back?
  }
...
// read register values into structure with gosipcmd
  ...
  // lemo output and reference channel:
   ...
   bool combine_or = ((control & COM_CTRL_COMBINE_OR_BIT) == COM_CTRL_COMBINE_OR_BIT ? true : false);

   theSetup->SetEnableTriggerReferenceChannel(hasrefchannel);
  ...
  QApplication::restoreOverrideCursor ();
   ...
   bool TamexPadiGui::WriteDAC_Padi (uint8_t chan, uint16_t value_padi[TAMEX_PADI_NUMCHIPS])
    ...
    double TamexPadiGui::GetVoltageFromRegister(unsigned int regval)
    {
      double rev=0;
      double m=1200.0/1024.0; // mV per reg count
      rev=-600.0 + m*regval;
      if(!fShowAmplifiedVoltages) rev/=200.0;
      //std::cout<<"GetVoltageFromRegister converts "<<regval<<" to "<< rev<<" mV" <<std::endl;
      return rev;
    }

    unsigned int TamexPadiGui::GetRegisterFromVoltage(double voltage)
    {
      unsigned int rev=0;
      double m=1024.0/1200.0; // reg count per mV
      if(!fShowAmplifiedVoltages) m*=200.0;
      rev= m*voltage + 511;
      //std::cout<<"GetRegisterFromVoltage converts "<<voltage<<" mV to "<< rev <<std::endl;
      return rev;
    }

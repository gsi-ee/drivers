...
// JAM: example how to adjust mbs readout with mbspex lib methods
...
#define DATA_FILT_CONTROL_DAT 0x84         // (0x80 E,t summary always +  data trace                 always
...
//--------------------------------------------------------------------------------------------------------
static long l_sfp0_feb_ctrl0[MAX_SLAVE] = { 0x01000000, 0x01000000, 0x01000000, 0x01000000,    // 12 bit/60 MHz
                                            0x01000000, 0x01000000, 0x01000000, 0x01000000,
                                            0x01000000, 0x01000000, 0x01000000, 0x01000000,
                                            0x01000000, 0x01000000, 0x01000000, 0x01000000 };

static long l_sfp1_feb_ctrl0[MAX_SLAVE] = { 0x92000000, 0x92000000, 0x92000000, 0x92000000,    // 14 bit/50 MHz
                                            0x92000000, 0x92000000, 0x92000000, 0x92000000,
                                            0x92000000, 0x92000000, 0x92000000, 0x92000000,
                                            0x92000000, 0x92000000, 0x92000000, 0x92000000 };

static long l_sfp2_feb_ctrl0[MAX_SLAVE] = { 0x81000000, 0x81000000, 0x81000000, 0x81000000,    // 14 bit/50 MHz
                                            0x81000000, 0x81000000, 0x81000000, 0x81000000,
                                            0x81000000, 0x81000000, 0x01000000, 0x01000000,
                                            0x81000000, 0x81000000, 0x81000000, 0x81000000 };

static long l_sfp3_feb_ctrl0[MAX_SLAVE] = { 0x01000000, 0x01000000, 0x01000000, 0x01000000,    // 12 bit/60 MHz
                                            0x01000000, 0x01000000, 0x01000000, 0x01000000,
                                            0x01000000, 0x01000000, 0x01000000, 0x01000000,
                                            0x01000000, 0x01000000, 0x01000000, 0x01000000 };
//--------------------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------------------
static long l_sfp0_feb_ctrl1[MAX_SLAVE] = { 0x1ffff, 0x1ffff, 0x1ffff, 0x1ffff,
                                            0x1ffff, 0x1ffff, 0x1ffff, 0x1ffff,
                                            0x1ffff, 0x1ffff, 0x1ffff, 0x1ffff,
                                            0x1ffff, 0x1ffff, 0x1ffff, 0x1ffff };

static long l_sfp1_feb_ctrl1[MAX_SLAVE] = { 0x1ffff, 0x1ffff, 0x1ffff, 0x1ffff,
                                            0x1ffff, 0x1ffff, 0x1ffff, 0x1ffff,
                                            0x1ffff, 0x1ffff, 0x1ffff, 0x1ffff,
                                            0x1ffff, 0x1ffff, 0x1ffff, 0x1ffff };
...
static long l_sfp3_feb_ctrl1[MAX_SLAVE] = { 0x1ffff, 0x1ffff, 0x1ffff, 0x1ffff,
                                            0x1ffff, 0x1ffff, 0x1ffff, 0x1ffff,
                                            0x1ffff, 0x1ffff, 0x1ffff, 0x1ffff,
                                            0x1ffff, 0x1ffff, 0x1ffff, 0x1ffff };
//--------------------------------------------------------------------------------------------------------
...
#define SEQUENTIAL_TOKEN_SEND 1        // - token sending and receiving is
 ...
 #define USED_TLU_FIFO        0        // vetar:   0
                                       // pexaria: 0
                                       // exploder on ttl/nim input 1: 16
#endif // WR_TIME_STAMP
...
static   INTU4    l_sfp_pat = 0;
         // file descriptor for PEXOR device
...
static  long              l_eb_first1=0, l_eb_first2=0;  
 ...
 static  int               nDevices;
 static  eb_data_t         eb_data2, eb_data3, eb_stat_before, eb_stat_after;
...
int f_user_get_virt_ptr (long  *pl_loc_hwacc, long  pl_rem_cam[])
{
  ...
  if (l_eb_first1 == 0)
  {
    ...
    {
      printm ("ERROR>> etherbone eb_open_socket, status: %s \n", eb_status(eb_stat));
    }

    if ((eb_stat = eb_device_open(eb_socket, WR_DEVICE_NAME, EB_ADDR32|EB_DATA32, 3, &eb_device)) != EB_OK)
    {
      printm ("ERROR>> etherbone eb_device_open, status: %s \n", eb_status(eb_stat));
    }
    ...
    {
      printm ("ERROR>> etherbone TLU eb_sdb_find_by_identity, status: %s \n", eb_status(eb_stat));
    }
  
    if (nDevices == 0)
    {
      printm ("ERROR>> etherbone no TLU found, status: %s \n", eb_status(eb_stat));
    }

    if (nDevices > 1)
    {
      printm ("ERROR>> etherbone too many TLUsfound, status: %s \n", eb_status(eb_stat));
    }
...

{
  ...
  if (l_eb_first2 == 0)
  {
    ...
    {
      printm ("ERROR>> etherbone TLU eb_device_write (CLEAR TLU FIFOS), status: %s \n", eb_status(eb_stat));
    }
    ...
    if ((eb_stat = eb_device_write(eb_device, wrTLU + GSI_TM_LATCH_TRIG_ARMSET,
                                          EB_BIG_ENDIAN|EB_DATA32, 0xFFFFFFFF, 0, eb_block)) != EB_OK)
    {
      printm ("ERROR>> etherbone TLU eb_device_write (ARM LATCHING), status: %s \n", eb_status(eb_stat));
    }
  }
  #endif // WR_TIME_STAMP
...
//printm ("pl_dat before readout: 0x%x \n", pl_dat); 
  ...
  *l_read_stat = 0;               
  ...
  if (bh_trig_typ < 14)
  {
    *pl_dat++ = SUB_SYSTEM_ID;  *l_se_read_len =+ 4;
    ...
    {
      printm ("ERROR>> etherbone EP eb_cycle_open, status: %s \n", eb_status(eb_stat));
    }
    ...
    eb_cycle_read(eb_cycle, wrTLUFIFO + GSI_TM_LATCH_FIFO_FTSHI, EB_BIG_ENDIAN|EB_DATA32, &eb_data2);
    /* read low word of latched timestamp */
    eb_cycle_read(eb_cycle, wrTLUFIFO + GSI_TM_LATCH_FIFO_FTSLO, EB_BIG_ENDIAN|EB_DATA32, &eb_data3);
    /* pop timestamp from FIFO */
    eb_cycle_write(eb_cycle, wrTLUFIFO + GSI_TM_LATCH_FIFO_POP, EB_BIG_ENDIAN|EB_DATA32, 0xF);
     ...
     * Upon termination, data and eb_data2 will be valid.
     * For higher performance, use multiple asynchronous cycles in a pipeline.
     */

    if ((eb_stat = eb_cycle_close(eb_cycle)) != EB_OK)
    {
      printm ("ERROR>> etherbone EP eb_cycle_close, status: %s \n", eb_status(eb_stat));
    }

    if ((eb_stat_before & l_used_tlu_fifo) != l_used_tlu_fifo)
    {
      printm ("ERROR>> TLU fifo %d is empty before time stamp read, stat: 0x%x\n", USED_TLU_FIFO, eb_stat_before);
    }
    if ((eb_stat_after & l_used_tlu_fifo) != 0)
    {
      //printm ("ERROR>> TLU fifo %d is not empty after time stamp read, stat: 0x%x\n", USED_TLU_FIFO, eb_stat_after);
    }
    // least significant bit of latched white rabbit time stamp represents 8 ns (125 MHz)
    // if 1 ns granualrity is required time stamp(s) must be shifted by 3 to left (<<3)
 
    //l_time_l16 =  (eb_data3<<3) & 0xffff;
    //l_time_m16 = ((eb_data3<<3) & 0xffff0000) >> 16 ;
    //l_time_h16 =  (eb_data2<<3) & 0xffff;
    l_time_l16 =  (eb_data3) & 0xffff;
    l_time_m16 = ((eb_data3) & 0xffff0000) >> 16 ;
    l_time_h16 =  (eb_data2) & 0xffff;
    ...
    *l_se_read_len += 3 * 4;
    ...
    ll_timestamp = (unsigned long long)eb_data2;
    ll_timestamp = (ll_timestamp << 32);
    ll_timestamp = ll_timestamp + (unsigned long long)eb_data3;
  ...
  *pl_dat++ = l_pola[3];
  ...
  switch (bh_trig_typ)
  {
    ...
    if ((bh_trig_typ != 14) && (bh_trig_typ != 15))
    {
      ...
      for (l_i=0; l_i<MAX_SFP; l_i++)
      {
        if (l_sfp_slaves[l_i] != 0)
        {
          ...
          /* set target to begin of pipe memory*/
    ...
    if (l_dat != TS__ID_L16)
    {
      printm (RON"ERROR>>"RES" 2. data word does not contain low 16bit identifier: %d \n");
      printm ("should be: 0x%x, but is: 0x%x\n", TS__ID_L16, l_dat);
    }
    l_dat = (*pl_tmp++) >> 16;
    if (l_dat != TS__ID_M16)
    {
      printm (RON"ERROR>>"RES" 3. data word does not contain low 16bit identifier: %d \n");
      printm ("should be: 0x%x, but is: 0x%x\n", TS__ID_M16, l_dat);
    }
    l_dat = (*pl_tmp++) >> 16; 
    if (l_dat != TS__ID_H16)
    {
      printm (RON"ERROR>>"RES" 4. data word does not contain low 16bit identifier: %d \n");
      printm ("should be: 0x%x, but is: 0x%x\n", TS__ID_H16, l_dat);
    } 
    #endif // WR_TIME_STAMP

...
char *argv[];
{
  ...
  else
  {
    ...
    if (l_n_feb > 1)
    {
      printf ("use flash on sfp %d on all febex3/tamex from id %d to %d \n",
                               l_sfp_id, l_fir_feb_id, l_las_feb_id);
    }
    else
    {
      printf ("use flash on sfp %d on febex3/tamex id %d \n", l_sfp_id, l_fir_feb_id);
    }
  ...
  if (l_stat == -1)
  {
    printf ("ERROR>> febex initialization failed on sfp %d \n", l_sfp_id);
    printf ("exiting...\n"); 
    exit (0); 
  }  
  ...
  l_end = 0;
  for (l_i = 0; l_i < 2 * FLASH_PAGE_SIZE; l_i++)
  {
    if (memcmp (pc_im_buf + l_i, "Bitstream CRC: 0x", 17) == 0)
    {
      l_end = l_i + 22;
      break;
    }
  ...
  if (l_end == 0)
  {
    printf("Error>> invalid firmware, CRC not found, exiting..\n ");
  }
  for (l_i = 0; l_i<l_end; l_i++) { pc_im_buf[l_i] = 0xff; }
  ...
  if ( (l_ec == 0) && (l_nv == 0) && (l_vo == 0) ) //standard
  {
    ...
    for (l_i=l_fir_feb_id; l_i<=l_las_feb_id; l_i++)
    {
      printf ("FEBEX3 SFP %d, ID %2d: flash loading and verification successfully finished \n", l_sfp_id, l_i);
    }
  }
  ...
  if ( (l_ec == 0) && (l_nv == 1) )
  {
    ...
    for (l_i=l_fir_feb_id; l_i<=l_las_feb_id; l_i++)
    {
      printf ("FEBEX3 SFP %d, ID %2d: flash successfully programmed. no verification \n", l_sfp_id, l_i);
    }
  }
  ...
  if ( (l_ec == 1) && (l_nv == 0))
  {
    ...
    for (l_i=l_fir_feb_id; l_i<=l_las_feb_id; l_i++)
    {
      ...
      printf ("FEBEX3 SFP %d, ID %2d: erase finished\n", l_sfp_id, l_i);      
    }
    l_ec_running = 0;
    for (l_i=l_fir_feb_id; l_i<=l_las_feb_id; l_i++)
    {
  ...
  if ( (l_ec == 1) && (l_nv == 1))
  {
    ...
    for (l_i=l_fir_feb_id; l_i<=l_las_feb_id; l_i++)
    {
      printf ("FEBEX3 SFP %d, ID %2d: flash loading successfully finished. no verifcation \n", l_sfp_id, l_i);
    }
  }
  ...
  if (l_vo == 1)
  {
    ...
    for (l_i=l_fir_feb_id; l_i<=l_las_feb_id; l_i++)
    {
      printf ("FEBEX3 SFP %d, ID %2d: flash verification successfully finished \n", l_sfp_id, l_i);
    }
  }
return 0;
}
...
void f_flash_check_write_in_progress (int l_feb_ident)
{
  int l_ct1 = 0;
  int l_ct2 = 0;
  ...
  while (l_wip == 1)
  {
    ...
    //usleep (250000); 
    //read flash status register
    f_pex_slave_wr (l_sfp_id, l_feb_ident, 0x300010, 0x0);    
    f_pex_slave_wr (l_sfp_id, l_feb_ident, 0x300018, 0x1001); 
    f_pex_slave_wr (l_sfp_id, l_feb_ident, 0x300018, 0x0);    
    f_pex_slave_wr (l_sfp_id, l_feb_ident, 0x300010, 0x5000000); 
    f_pex_slave_wr (l_sfp_id, l_feb_ident, 0x300018, 0x1000);    
    f_pex_slave_wr (l_sfp_id, l_feb_ident, 0x300018, 0x0);
    f_pex_slave_wr (l_sfp_id, l_feb_ident, 0x300008, 0x100);     
    l_data = 0;
    f_pex_slave_rd (l_sfp_id, l_feb_ident, 0x300004, &l_data);
    if ((l_data & 0x1) == 0) {l_wip = 0;}   
    //printf ("flash status after block %2d erased:       sfp %d, febex %d: 0x%x \n", l_j, l_sfp_id, l_feb_ident, l_data & 0xff);
...
void f_flash_unprotect (int l_feb_id)
{
  // flash write enable
  f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300010, 0x6000000); // spi commando RDID to data in 
  f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300018, 0x1000);    // spi write en high for addr 0 - control register
  f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300018, 0x0);       // spi write enable low

  //read flash status register
  f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300010, 0x0);        // max data word to spi data in  
  f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300018, 0x1001);     // spi write en high for addr 1 - status register  
  f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300018, 0x0);        // spi write en low 
  f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300010, 0x5000000);  // spi commando: read flash status register 
  f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300018, 0x1000);     // spi write en high for addr 0 - control register
  f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300018, 0x0);
  f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300008, 0x100);      // memory read command + address
  l_data = 0;
  f_pex_slave_rd (l_sfp_id, l_feb_id, 0x300004, &l_data);    // read memory data out 
  //printf ("flash status after flash write enable:    sfp %d, febex %d: 0x%x \n", l_sfp_id, l_feb_id, l_data & 0xff);

  // write 0 to flash status register (unprotect all sectors in trb code)

  // flash write enable
  f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300010, 0x6000000);
  f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300018, 0x1000);
  f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300018, 0x0);
  // write 0 to spi mem
  f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300000, 0x0);        
  f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300008, 0x1000);    
  f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300008, 0x0);       
  // write flash status
  f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300010, 0x1000000); 
  f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300018, 0x1000); 
  f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300018, 0x0);    

  f_flash_check_write_in_progress (l_feb_id);
}

/*****************************************************************************/

void f_flash_block_erase (int l_feb_id)
{
  // erase first 4 MB of flash (64 blocks x 0x10000 bytes), flash total: 8 MB
  printf ("\nFEBEX3 SFP %d, ID %2d: start erasing first 4 MB (out of 8 MB) in flash memory \n", l_sfp_id, l_feb_id);
  for (l_j=0; l_j<64; l_j++)
  {
    // flash write enable
    f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300010, 0x6000000);
    f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300018, 0x1000);
    f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300018, 0x0);
    // erase block
    f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300010, 0xd8000000 + (l_j * FLASH_BLOCK_SIZE));
    f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300018, 0x1000);
    f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300018, 0x0);
    ...
    if (((l_j%10) == 0) && (l_j !=0) )
    { 
      printf ("FEBEX3 SFP %d, ID %2d: %3d blocks of 64 Kbytes erased \n", l_sfp_id, l_feb_id, l_j);
    }
  }
  printf ("FEBEX3 SFP %d, ID %2d: %3d blocks of 64 Kbytes erased \n", l_sfp_id, l_feb_id, l_j);
  printf ("FEBEX3 SFP %d, ID %2d: erase finished\n", l_sfp_id, l_feb_id);
...
/*****************************************************************************/
void f_flash_chip_erase (int l_feb_id)
{
  printf ("FEBEX3 SFP %d, ID %2d: start erasing full flash memory (needs about a minute)\n", l_sfp_id, l_feb_id);
  // flash write enable
  f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300010, 0x6000000);
  f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300018, 0x1000);
  f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300018, 0x0);
  // erase chip
  f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300010, 0xc7000000);
  f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300018, 0x1000);
  f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300018, 0x0);
}

/*****************************************************************************/
void f_flash_load (int l_feb_id)
{
  // load flash
  printf ("\nFEBEX3 SFP %d, ID %2d: start loading fpga bit file into flash memory \n", l_sfp_id, l_feb_id);
  ...
  for (l_j=0; l_j<l_im_n_pages; l_j++)
  {
    ...
    {
      f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300000, *pl_im_tmp++);       
      f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300008, 0x1000 + l_k);
      f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300008, 0x0);           
    }
    ...
    f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300010, l_spi_trans_size_code);
    f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300018, 0x1001);
    f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300018, 0x0);
    ...
    f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300010, 0x2000000 + l_j * FLASH_PAGE_SIZE);
    f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300018, 0x1000);
    f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300018, 0x0);
    ...
    if (((l_j%1000) == 0) && (l_j !=0) )
    { 
      printf ("FEBEX3 SFP %d, ID %2d: %5d pages of 256 bytes loaded \n", l_sfp_id, l_feb_id, l_j);
    }
  } 
  printf ("FEBEX3 SFP %d, ID %2d: %5d pages of 256 bytes loaded \n", l_sfp_id, l_feb_id, l_j);
  printf ("FEBEX3 SFP %d, ID %2d: loading finished \n", l_sfp_id, l_feb_id);
}

/*****************************************************************************/
void f_flash_readback_verify (int l_feb_id)
{
  int l_ct = 0;
  // read back flash and compare with image buffer from input file
  printf ("\nFEBEX3 SFP %d, ID %2d: start reading flash memory \n", l_sfp_id, l_feb_id);
  ...
  for (l_j=0; l_j<l_im_n_pages; l_j++)
  {
    ...
    f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300010, 0x3000000 + l_j * FLASH_PAGE_SIZE);
    f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300018, 0x1000);
    f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300018, 0x0);
    ...
    while (l_data == 1)
    {
      f_pex_slave_rd (l_sfp_id, l_feb_id, 0x30001c, &l_data);
      l_data = l_data & 1;
      l_ct++;
      if (l_ct > 3)
      {
    ...
    for (l_k=0; l_k<64; l_k++)
    {
      f_pex_slave_wr (l_sfp_id, l_feb_id, 0x300008, 0x100 + l_k);     
      f_pex_slave_rd (l_sfp_id, l_feb_id, 0x300004, &l_data);
    ...
    if (((l_j%1000) == 0) && (l_j !=0) )
    { 
      printf ("FEBEX3 SFP %d, ID %2d: %5d pages of 256 bytes read back and verified with input file, ok!\n", l_sfp_id, l_feb_id, l_j);
    }
  } 
  printf ("FEBEX3 SFP %d, ID %2d: %5d pages of 256 bytes read back and verified with input file, ok!\n", l_sfp_id, l_feb_id, l_j);
  printf ("FEBEX3 SFP %d, ID %2d: verification finished \n", l_sfp_id, l_feb_id);
}
...
{
  printf ("\n*** %s v 1.0 02-June 2020 by NK, JAM (GSI EEL department) *** \n", progname);
  printf ("\nusage of %s,  < > are mandatory parameters   \n\n",progname);
  printf ("%s <sfp id> <first febex3/tamex id> <# of febex3/tamex> <bitfile input fn> \n",progname); 
  printf ("%s <sfp id> <first febex3/tamex id> <# of febex3/tamex> <bitfile input fn> -nv \n",progname); 
  printf ("%s <sfp id> <first febex3/tamex id> <# of febex3/tamex> <bitfile input fn> -ec\n",progname); 
  printf ("%s <sfp id> <first febex3/tamex id> <# of febex3/tamex> <bitfile input fn> -ec -nv \n",progname);
  printf ("%s <sfp id> <first febex3/tamex id> <# of febex3/tamex> <bitfile input fn> -vo \n,progname",progname);  
  printf ("\n");
  printf ("no option: block erase, load, verify \n");
  printf (" -nv     : no verify \n");
  printf (" -ec     : chip  erase (parallel erase, faster if more than 2 febex3/tamex specified) \n");

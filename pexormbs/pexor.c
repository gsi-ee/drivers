...
ssize_t pexor_sysfs_trixorregs_show(struct device *dev,
        struct device_attribute *attr, char *buf);
ssize_t pexor_sysfs_bar0base_show(struct device *dev,
        struct device_attribute *attr, char *buf);
ssize_t pexor_sysfs_trixorbase_show(struct device *dev,
        struct device_attribute *attr, char *buf);
...
static DEVICE_ATTR(bar0base, S_IRUGO, pexor_sysfs_bar0base_show, NULL);
static DEVICE_ATTR(trixorbase, S_IRUGO, pexor_sysfs_trixorbase_show, NULL);
#endif
...
int pexor_open(struct inode *inode, struct file *filp)
{
    ...
    pexor_dbg(KERN_INFO "END   pexor_open \n");
    return 0;                   // success
}
//-----------------------------------------------------------------------------
int pexor_release(struct inode *inode, struct file *filp)
{
    pexor_dbg(KERN_INFO "BEGIN pexor_release \n");
    pexor_dbg(KERN_INFO "END   pexor_release \n");
    return 0;
}
    ...
    if (vma->vm_pgoff == 0)
        {
            ...
            pexor_dbg(KERN_NOTICE "Pexor is Mapping bar0 base address %x / PFN %x\n", privdata->l_bar0_base, privdata->l_bar0_base >> PAGE_SHIFT);
        ...
        unsigned long arg)
{
    int retval = 0;
    struct pexor_privdata *privdata;
    pexor_dbg((KERN_INFO "BEGIN pexor_ioctl \n"));
    privdata = (struct pexor_privdata*) filp->private_data;
    switch (cmd)
        {
        case WAIT_SEM:
        ...
        case GET_BAR0_BASE:
            ...
            pexor_dbg(KERN_INFO " after  GET_BAR0_BASE \n");
            break;
        case GET_BAR0_TRIX_BASE:
            pexor_dbg(KERN_INFO " before GET_TRIX_BASE \n");
            retval = __put_user(privdata->l_bar0_trix_base, (int __user *)arg);
            pexor_dbg(KERN_INFO " after  GET_TRIX_BASE \n");
            break;
        case RESET_SEM:
...
//-----------------------------------------------------------------------------
struct file_operations pexor_fops = { .owner = THIS_MODULE,
...
irqreturn_t irq_hand(int irq, void *dev_id)
{
    ...
    struct pexor_privdata *privdata;
    pexor_dbg(KERN_INFO "BEGIN irq_hand \n");
    ...
    pexor_dbg(KERN_INFO "END   irq_hand \n");
...
static int probe(struct pci_dev *dev, const struct pci_device_id *id)
{
    ...
    if ((err = pci_enable_device(dev)) != 0)
        {
            pexor_msg(
                    KERN_ERR "PEXOR pci driver probe: Error %d enabling PCI device! \n", err);
            return -ENODEV;
        }
...
// here set custom registers for mbs implementation:
    ...
    if ((err = pci_read_config_byte(dev, PCI_INTERRUPT_PIN, &(privdata->irqpin)))
        ...
        {
            pexor_msg(
                    KERN_ERR "PEXOR pci driver probe: Error %d getting the PCI interrupt pin \n", err);
        }
    ...
    if (!IS_ERR(pexor_class))
        {
            ...
            if (device_create_file(privdata->class_dev, &dev_attr_codeversion)
                ...
                {
                    pexor_msg(
                            KERN_ERR "Could not add device file node for code version.\n");
                }

            if (device_create_file(privdata->class_dev, &dev_attr_trixorregs)
                    != 0)
                {
                    pexor_msg(
                            KERN_ERR "Could not add device file node for trixor registers.\n");
                }
            if (device_create_file(privdata->class_dev, &dev_attr_bar0base)
                    != 0)
                {
                    pexor_msg(
                            KERN_ERR "Could not add device file node for bar0 base address.\n");
                }
            if (device_create_file(privdata->class_dev, &dev_attr_trixorbase)
                    != 0)
                {
                    pexor_msg(
                            KERN_ERR "Could not add device file node for trixor base address.\n");
                }
...
//-----------------------------------------------------------------------------
static struct pci_driver pci_driver = { .name = PEXORNAME, .id_table = ids,
...
static int __init pexor_init(void)
{
    ...
    if (my_major_nr)
        {
            result = register_chrdev_region(pexor_devt, PEXOR_MAXDEVS,
                    PEXORNAME);
        }
    else
        {
            result = alloc_chrdev_region(&pexor_devt, 0, PEXOR_MAXDEVS,
                    PEXORNAME);
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,18)
    if (pexor_class != NULL ) class_destroy(pexor_class);
#endif

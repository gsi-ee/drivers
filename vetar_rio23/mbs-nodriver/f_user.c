...
//      -> f_vetar_init();
//          ->f_a32_vme_mas_map_CRCSR() = map crcsr space of VETAR to user space
//          * set ADER registers to define address windows for a24 (wishbone control) and a32 (tlu access)
//          -> f_a24_vme_mas_map_CONTROL() = map a24 control window to user space
//          * write a24 control registers to setup direct access of TLU via a32 register
//      ->f_a32_vme_mas_map_TLU(= map a32 registers of TLU to user space
////
...
// JAM 18-03-24: extend for rio2/3 test


// switch to IFC mapping
//#define USE_IFC 1


// enable this for RIO2/3 mapping on lynx OS
#define USE_RIO_LYNX 1
...
void f_a32_vme_mas_map_TLU ();
void f_a32_vme_mas_free_TLU ();
...
void f_a32_vme_mas_map_TLU ()
{
  ...
  if (alt_fd <= 0)
  {
    printm ("f_a32_vme_mas_map_TLU - Error: could not do alt_init\n");
    exit (-1);
  }
  printm ("f_a32_vme_mas_map_TLU...\n");
  ...
  if (pl_virt_vme_base_tlu == MAP_FAILED)
  {
    ...
    vmeaddr = VETAR_REGS_ADDR;
    vme_len = VETAR_REGS_SIZE;
    vme_am = 0x09;
    pl_virt_vme_base_tlu  = (INTU4*) find_controller(vmeaddr,vme_len,vme_am,0,0,&param);
    if( pl_virt_vme_base_tlu ==  (INTU4*) -1 )
    {
        printm ("f_map_a32_mas_vme ERROR>> find_controller() failed, cannot get user space address. exiting...\n");
        exit (-1);
    }
...
#ifdef  USE_RIO_LYNX
  return_controller(pl_virt_vme_base_tlu,VETAR_REGS_SIZE);
...
/*****************************************************************************/
void f_a24_vme_mas_map_CONTROL ()
{
#ifdef  USE_IFC
  int triva_fd = 0;
  ...
  if (alt_fd <= 0)
  {
    printm ("f_a24_vme_mas_map_CONTROL - Error: could not do alt_init\n");
    exit (-1);
  }
  printm ("f_a24_vme_mas_map_CONTROL...\n");
  ...
  {
    printm ("f_a24_vme_mas_map_CONTROL ERROR>> alt_map_alloc failed, exiting..\n");
    exit (-1);
  }
  ...
  {
    printm (
        "f_a24_vme_mas_map_CONTROL ERROR>> mmap failed, cannot get user space address at len 0x%x, offset 0x%x.. exiting... Error>> %d (%s)\n",
  ...
  // TODO: meaning of this parameters?
    ...
    param.wrpost = 0;
    param.swap = SINGLE_AUTO_SWAP;
    vmeaddr = VETAR_CTRL_ADDR;
    vme_len = VETAR_CTRL_SIZE;
    vme_am = 0x39;//VME_AM_A24 | VME_AM_DATA;
    pl_virt_vme_base_control  = (INTU4*) find_controller(vmeaddr,vme_len,vme_am,0,0,&param);
    if( pl_virt_vme_base_control ==  (INTU4*) -1 )
    {
        printm ("f_a24_vme_mas_map_CONTROL ERROR>> find_controller() failed, cannot get user space address. exiting...\n");
        exit (-1);
    }
...
void f_a24_vme_mas_free_CONTROL ()
{
...
#ifdef  USE_RIO_LYNX
  return_controller(pl_virt_vme_base_control,VETAR_CTRL_SIZE);
#endif

}

/*****************************************************************************/
void f_a32_vme_mas_map_CRCSR ()
{
#ifdef  USE_IFC
  int triva_fd = 0;
  long offset = 0, len = 0;
  ...
  if (alt_fd <= 0)
  {
    printm ("f_a32_vme_mas_map_CRCSR - Error: could not do alt_init\n");
    exit (-1);
  }
  printm ("f_a32_vme_mas_map_CRCSR...\n");
  ...
  {
    printm (
        "f_a32_vme_mas_map_CRCSR ERROR>> mmap failed, cannot get user space address. exiting.. Error>> %d (%s)\n",
        errno, strerror (errno));
    exit (-1);
  }
...
#ifdef  USE_RIO_LYNX
  return_controller(pl_virt_vme_base_crcsr,VETAR_CRCSR_SIZE);

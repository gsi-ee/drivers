...
void GalChannelWidget::ReadSettings (QSettings* settings)
{
// TODO anything here?

}

void GalChannelWidget::WriteSettings (QSettings* settings)
{
// TODO anything here?
}
...
void GalChannelWidget::EvaluateView ()
{
  ...
  for (uint8_t channel = 0; channel < GAPG_CHANNELS; ++channel)
  {
    ...
    if (!rev)
      printm ("Evaluate View Warning: sequence %s of current channel %d not known", seqname, channel);

    const char* patname = fChannelPatternCombo[channel]->currentText ().toLatin1 ().data ();
    rev = theSetup->SetChannelPattern (channel, patname);
    if (!rev)
      printm ("Evaluate View Warning: pattern %s of current channel %d not known", patname, channel);
  }
...
// setup combobox entries from known sequences:
  ...
  for (int six = 0; six < theSetup->NumKnownSequences (); ++six)
  {
    ...
    {
      if (six == 0)
        fChannelSequenceCombo[channel]->clear ();
...
// std::cout << "GalChannelWidget::ApplyChannelSequence chan="<<channel<<",  ix="<<ix << std::endl;
  ...
  bool rev = theSetup->SetChannelSequence (channel, seqname);
  if (!rev)
    printm ("ApplyChannelSequence Warning: sequence %s of current channel %d not known", seqname, channel);
  theSetup->SetChannelSequence (channel, seqname);
  fParent->WriteGAPG ( GAPG_CHANNEL_SEQUENCE_BASE + channel * sizeof(uint32_t),
      theSetup->GetChannelSequenceID (channel));
}
...
void GalChannelWidget::ApplyChannelPattern (int channel, int ix)
{
  ...
  if (!rev)
    printm ("ApplyChannelPattern Warning: pattern %s of current channel %d not known", patname, channel);

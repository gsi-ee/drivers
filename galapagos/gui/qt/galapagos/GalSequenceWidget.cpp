    ...
    gapg::BasicObjectEditorWidget (parent)
{
  ...
  Object_comboBox->setToolTip("Select known sequence by name");
  ObjectIDSpinBox-> setToolTip("Unique id of selected sequence");

  ObjectNewButton->setToolTip("Create new Sequence");
  ObjectEditButton->setToolTip("Edit contents of selected sequence");
  ObjectDeleteButton->setToolTip("Remove selected sequence");
  ObjectLoadButton->setToolTip("Load sequence from *.gas file to the list");
  ObjectSaveButton->setToolTip("Export selected sequence to a file (*.gas)");
  ObjectEditCancelButton->setToolTip("Cancel edited code for selected sequence (restore last setup)");
  ObjectApplyButton->setToolTip("Apply editor contents for selected sequence");
...
bool GalSequenceWidget::NewObjectRequest ()
{
  ...
  while (theSetup->GetSequence (sid) != 0)
    sid++;

  QString defaultname = QString ("Sequence_%1").arg (sid);
  QString seqname = QInputDialog::getText (this, tr ("Create a new sequence"), tr ("Sequence name:"), QLineEdit::Normal,
      defaultname, &ok);
...
bool GalSequenceWidget::LoadObjectRequest ()
{
  ...
  QFileDialog fd (this, "Select Files with New Galapagos command sequence", fLastFileDir,
...
bool GalSequenceWidget::SaveObjectRequest ()
{
  ...
  QFileDialog fd (this, "Save Galapagos command sequence to file", fLastFileDir,
...
bool GalSequenceWidget::LoadObject (const QString& fileName)
{
  ...
  {
    printm ("LoadSequence %s error: sequence %s had already assigned specified unique id %d !",
...
bool GalSequenceWidget::SaveObject (const QString& fileName, BasicObject* ob)
{
  ...
  QString header = QString ("#Sequence %1 saved on %2").arg (seq->Name ()).arg (
...
void GalSequenceWidget::RefreshView ()
{
  ...
  if (oldsid >= theSetup->NumKnownSequences ())
    oldsid = theSetup->NumKnownSequences () - 1;
